 ...
 * 本地种子数据执行脚本
 *
 * 使用 Wrangler D1 直接执行 SQL 插入种子数据
 *
 * 使用方法:
 * pnpm db:seed        - 插入种子数据
...
async function runSQL(sql: string, local = true) {
  const dbArg = local ? '--local' : '';
  const cmd = `wrangler d1 execute cf-blog-db ${dbArg} --file -`;
...
async function main() {
  ...
  try {
    if (command === 'clear') {
      ...
      await runSQL(clearSQL, !isRemote);
      console.log('✅ 数据清除完成！');
    } else {
      console.log('📝 开始插入种子数据...');
      await runSQL(seedSQL, !isRemote);
